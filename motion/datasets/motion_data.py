...
class MotionDataset(Dataset):
    ...
    def __init__(self, control_data, joint_data, labels, metas, seqlen, n_lookahead, dropout):
        """
        Args:
        ...
        # self.autoreg = np.swapaxes(self.autoreg, 1, 2)
    ...
    def __getitem__(self, idx):
        ...
        if self.dropout > 0.:
            ...
            sample = {'x': self.x[idx, :, :], 'cond': cond_masked, 'label': self.labels[idx],
                      'meta': self.metas[idx]}
        else:
            sample = {'x': self.x[idx, :, :], 'cond': self.cond[idx, :, :], 'autoreg': self.autoreg[idx, :, :], 'control': self.control[idx, :, :], 'label': self.labels[idx],
                      'meta': self.metas[idx]}
...
class TestDataset(Dataset):
    ...
    def __init__(self, control_data, joint_data, labels, metas):
        """
        Args:
        control_data: The control input
        joint_data: body pose input
    ...
    def __getitem__(self, idx):
        sample = {'x': self.autoreg[idx, :], 'cond': self.control[idx, :], 'label': self.labels[idx],
                  'meta': self.metas[idx]}
        return sample
