...
class Locomotion():
    ...
    def __init__(self, hparams):
        ...
        train_series = np.load(os.path.join((data_root + '_mxia'), 'all_locomotion_mxia_32.npz'), allow_pickle=True)['train'].item()
        (train_data, train_labels, train_metas) = (train_series['motion'], train_series['style'], train_series['meta'])
        train_data = np.array(train_data).astype(np.float32)
        test_series = np.load(os.path.join((data_root + '_mxia'), 'all_locomotion_mxia_32.npz'), allow_pickle=True)['test'].item()
        ...
        train_labels = train_metas['content'][:]
        ...
        self.train_dataset = MotionDataset(train_data[:, :, (- 3):], train_data[:, :, :(- 3)], train_labels, train_metas, hparams.Data.seqlen, hparams.Data.n_lookahead, hparams.Data.dropout)
        self.test_dataset = TestDataset(all_test_data[:, :, (- 3):], all_test_data[:, :, :(- 3)], test_labels, test_metas)
        self.validation_dataset = MotionDataset(validation_data[:, :, (- 3):], validation_data[:, :, :(- 3)], val_labels, val_metas, hparams.Data.seqlen, hparams.Data.n_lookahead, hparams.Data.dropout)
