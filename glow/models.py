...
def f(in_channels, out_channels, hidden_channels, cond_channels, network_model, num_layers):
    if network_model == 'Transformer':
        return modules.TransformerModel(out_channels, in_channels + cond_channels, 2, hidden_channels, 2, 0)


class FlowStep(nn.Module):
    FlowCoupling = ["additive", "affine"]
    NetworkModel = ["Transformer"]
    FlowPermutation = {
    ...
    def __init__(self, in_channels, hidden_channels, cond_channels,
        ...
        # 2. permute
        if flow_permutation == "invconv": # this path !
    ...
    def execute(self, input, cond, logdet=None, reverse=False):
        if not reverse:
...
class FlowNet(nn.Module):
    ...
    def execute(self, z, cond, logdet=0., reverse=False, eps_std=None):
        if not reverse:
...
class Glow(nn.Module):
    ...
    def __init__(self, x_channels, cond_channels, hparams):
        ...
        self.z_shape = [1, x_channels, 1]


    def execute(self, x=None, cond=None, z=None,
