...
class Trainer(object):
    def __init__(self, graph, optim, lrschedule, loaded_step,
        ...
        self.data_loader = self.train_dataset.set_attrs(batch_size=self.batch_size, shuffle=False) # change to false for debug
        ...
        #                                    shuffle=False,
        #                                    drop_last=True)
        self.test_data_loader = data.get_test_dataset().set_attrs(batch_size=100, shuffle=True)
        ...
        #                                   shuffle=False,
        #                                   drop_last=True)
        self.val_data_loader = data.get_validation_dataset().set_attrs(batch_size=self.batch_size, shuffle=True)
    ...
    def prepare_cond(self, jt_data, ctrl_data):
        ...
        cond = jt.array(np.expand_dims(np.concatenate((jt_data, ctrl_data), axis=1), axis=-1))
    ...
    def test_pair(self, stylebvh, contentbvh, eps_std=1.0, frames=1, counter=0):
        ...
        if style.shape[1] >= content.shape[1]  and style_label != content_label:
            ...
            autoreg_all = batch["x"].data
            control_all = batch["cond"].data
    ...
    def train(self):
        ...
        for epoch in range(self.n_epoches):
            ...
            for i_batch, batch in enumerate(progress):
                ...
                cond = jt.array(batch["cond"]) # 100, 663, 70
                ...
                if self.global_step == 0:
                    self.graph(x[:self.batch_size, ...],
                               cond[:self.batch_size, ...] if cond is not None else None)
                    # re-init LSTM hidden
                    # if hasattr(self.graph, "module"):
                    #     self.graph.module.init_lstm_hidden()
                    # else:
                ...
                loss_classes = 0
                if self.global_step % self.scalar_log_gaps == 0:
                    self.writer.add_scalar("loss/loss_generative", loss_generative.data, self.global_step)
                ...
                if self.global_step % self.validation_log_gaps == 0:
                    ...
                    self.writer.add_scalar("val_loss/val_loss_generative", loss_val.data, self.global_step)
