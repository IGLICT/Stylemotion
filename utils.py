...
def load(step_or_path, graph, optim=None, criterion_dict=None, pkg_dir="", device=None):
    ...
    # begin to load
    state = jt.load(save_path)
    global_step = state["global_step"]

    removeKeys = []
    for key in state["graph"].keys():          
        if "src_mask" in key:          
...
def __prepare_cond(autoreg, control, data_device):
    ...
    cond = jt.from_numpy(np.expand_dims(np.concatenate((autoreg,control),axis=1), axis=-1))
